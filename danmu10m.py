...
class Danmu():
    ...
    def get_danmu(self):
        ...
        for content in html['data']['room']:
            ...
            if msg + '\n' not in self.log:
                # 使用主线程更新GUI
                window.after(0, lambda m=msg: listb.insert(END, m))
                window.after(0, lambda: listb.see(END))
...
def save_danmu_impl(show_prompt=True):
    """弹幕保存实现函数"""
...
def start_point():
    try:
        ...
        if room_int is not None:
            ...
            t.start()
            # 启动自动保存循环
            window.after(600000, auto_save_loop)
    except ValueError:
